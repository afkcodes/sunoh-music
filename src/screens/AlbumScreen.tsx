...
const AnimatedLegendList = Animated.createAnimatedComponent(LegendList) as unknown as typeof LegendList;
          ...
          <View style={styles.metadata}>
            ...
            <View style={styles.metaRow}>
              <Text variant="caption" color="secondary">
                {decodeHtmlEntities(album?.year || '')}
              </Text>
              <View style={styles.dot} />
              <Text variant="caption" color="secondary">
                {album?.listCount || '0'}{' '}
                {parseInt(album?.listCount || '0') === 1 ? 'Song' : 'Songs'}
              </Text>
            </View>
          </View>
...
export const AlbumScreen: React.FC = () => {
  ...
  const { data: albumData, isLoading, error } = useAlbumData(albumId);
  const s = useScaling();

  const album = albumData?.data?.album;
  const sections = albumData?.data?.sections || [];

  // Use dataExtractor to safely get the image URL
  const imageUrl = useMemo(
    () =>
      dataExtractor<string>(
        album as unknown as NestedObject,
        dataConfigs.album.image
      ) || '',
    [album]
  );
  ...
  const listData = useMemo(() => {
    return (album?.list || []).map((song: Song) => ({
      type: 'song' as const,
      data: song,
      id: song.id,
    }));
    ...
    <SafeView style={styles.container} applyTopInset={false}>
      ...
      >
        ...
        >
          ...
          {/* Theme Icon (Scrolled) */}
          <Animated.View style={[StyleSheet.absoluteFill, { justifyContent: 'center', alignItems: 'center' }, iconThemeAnimatedStyle]}>
